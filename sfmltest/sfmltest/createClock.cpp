...
string ConvertIntToString(intValue value)
{
...
{
	int indexNum = i / 5;
	clock.num[indexNum].setFont(clock.font);
	clock.num[indexNum].setFillColor(Color::Black);
	clock.num[indexNum].setStyle(Text::Bold);
	clock.num[indexNum].setString(ConvertIntToString(digit));
	clock.num[indexNum].setCharacterSize(38);
	Vector2f digitPosition = GetPosition(angle, 50);
	Vector2f positionText(digitPosition + windowCenter);
	clock.num[indexNum].setPosition(positionText.x, positionText.y - clock.num[indexNum].getGlobalBounds().height / 2.0f);

	clock.num[indexNum].setOrigin(clock.num[indexNum].getGlobalBounds().width / 2.0f,
		clock.num[indexNum].getGlobalBounds().height / 2.0f);
...
void CreateClockFace(const Vector2f &windowCenter, ClockSystem &clock)
{
	...
	for (int i = 0; i < NUMBER_DOTS; ++i)
	{
		if (i % 5 == 0)
		{
			...
			CreateDigit(i, clock, angle, digit, windowCenter);
		}
		else
		{
